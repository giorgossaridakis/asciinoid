...
int main()
{
  ...
  srand(time(NULL));
   ...
   if (reset_flag) {
    ...
    gotoxy(9,1);
    textcolor(RED); printw("   ");	
    gotoxy(9,1);
    textcolor(RED); printw("%d",lives);
	gotoxy(21,1);
	textcolor(YELLOW) ; printw("    ");
	gotoxy(21,1);
	textcolor(YELLOW); printw("%d", level);
	gotoxy(39,1);
    textcolor(GREEN); printw( "    "); 
	gotoxy(39,1);
    textcolor(GREEN); printw("%d", tiles_left); 
	gotoxy(51,1); 
	textcolor(BLUE) ; printw("    ");
	gotoxy(51,1); 
	textcolor(BLUE) ; printw("%d", score);
// 	refresh();
	 
	/* erase racket, tiles, gifts and balls */
	for (n=0;n<racket.size;n++) {	
	  gotoxy(racket.pt.x+n, racket.pt.y);
	 textcolor(racket.color); printw(" "); } 
	for (i=0;i<tiles_level;i++) {
	if (!tiles[i].playability) {
	  gotoxy(tiles[i].pt.x, tiles[i].pt.y);
    textcolor(tiles[i].color); printw(" "); } }	 
	for (i=0;i<MAXGIFTS;i++) {
	 if (gifts[i].playability) {
	  gotoxy(gifts[i].pt.x, gifts[i].pt.y-1);
	 textcolor(gifts[i].color); printw(" "); } }
	for (i=0;i<MAXBALLS;i++) {
 	 if (balls[i].playability) {
	  gotoxy(balls[i].pt.x, balls[i].pt.y);
 	 textcolor(balls[i].color); printw(" ");  } }
    refresh();
    ...
    switch (c) {
	 ...
	 case 'p':
	  for (n=0;n<racket.size;n++) {	
	   gotoxy(racket.pt.x+n, racket.pt.y);
	   textcolor(racket.color); addch(racket.character); } 
	   refresh();
	  message(1, "paused");
	  c=getch();
	  message(0, "paused");
	...
	for (i=0;i<tiles_level;i++) {
	 if (tiles[i].playability) {
	  gotoxy(tiles[i].pt.x, tiles[i].pt.y);
     textcolor(tiles[i].color); addch(tiles[i].character); } } 
	for (i=0;i<MAXGIFTS;i++) {
	 if (gifts[i].playability) {
	  gotoxy(gifts[i].pt.x, gifts[i].pt.y);
	 textcolor(gifts[i].color); addch(gifts[i].character); } }
	for (i=0;i<MAXBALLS;i++) {
	if (balls[i].playability) {
	 gotoxy(balls[i].pt.x, balls[i].pt.y);
	textcolor(balls[i].color); addch(balls[i].character);  } } 
    refresh();
	...
	for (i=1;i<80;i++) {
     gotoxy(i,2);
	textcolor(CYAN); addch(BORDER); }
	/* show the racket for better display */
	for (n=0;n<racket.size;n++) {	
	  gotoxy(racket.pt.x+n, racket.pt.y);
	  textcolor(racket.color);
     addch(racket.character); }
    refresh();
	switch (level) {
	 case 1:
	  Sleep(150);
...
void draw_screen()
{
   int i;	
	...
	curs_set(0);
	gotoxy(1,1);
	textcolor(RED); printw("rackets:  ");
	gotoxy(15,1);
	textcolor(YELLOW); printw("level:  ");
	gotoxy(28,1);
    textcolor(GREEN); printw("tiles left:   "); 
	gotoxy(45,1); 
	textcolor(BLUE); printw("score:   ");
    gotoxy(67,1);
    textcolor(MAGENTA); printw("C++ ASCIINoid");
	gotoxy(2,24);
	textcolor(GREEN) ; printw(" | left/right arrow|+ to add balls|* level up|/ level down|. redraw|p pause | ");
...
void message(int flag, const char message[])
{
  int i;	
   ...
   if (flag) {
    gotoxy(35,12);	
    textcolor(YELLOW) ; printw("%s", message); }
   else { 
    gotoxy(35,12);	
    textcolor(BLACK);
    printw("%s", message); }
...
{ 
 ...
 int y=3;
  ...
  for (i=0;i<MAXTILES;i++) {
   ...
   tiles[i].character='=';
   tiles[i].color=8;
   while (tiles[i].color==8)
    tiles[i].color=rand() % 15 + 1;
...
int check_ball(struct ball_prototype *ball, char c)
{
  ...
  int r;
   ...
   ball->pt.y+=ball->direction.y; }
    ...
    ball->pt.x=(racket.pt.x+racket.size/2)+1;
     gotoxy(ball->pt.x, ball->pt.y);
	textcolor(ball->color) ; addch(ball->character); }
...
void reset_gifts()
{
  int i;
   ...
   for (i=0;i<MAXGIFTS;i++) {
	...
	gifts[i].pt.y=2;
    gifts[i].color=8;
    while (gifts[i].color==8)
	 gifts[i].color=rand() % 15 +1 ;
